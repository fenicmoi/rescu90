...
if ($id > 0) {
    $stmt = $pdo->prepare("SELECT * FROM target_types WHERE id = :id");
    $stmt->execute([':id' => $id]);
    $fetched = $stmt->fetch();
    if ($fetched) {
        $type = $fetched;
...
<body class="bg-gray-50 text-gray-800 flex flex-col min-h-screen">
    ...
    <main class="flex-grow p-6 lg:p-8 max-w-2xl mx-auto w-full">
        <div class="mb-6">
            <h1 class="text-2xl font-bold text-gray-800"><?= $is_edit ? 'แก้ไขประเภทบ้านเป้าหมาย' : 'เพิ่มประเภทบ้านเป้าหมายใหม่' ?></h1>
        </div>

        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
            <form action="save_target_type.php" method="POST">
                <input type="hidden" name="id" value="<?= $type['id'] ?>">
                
                <div class="mb-6">
                    <label for="type_name" class="block text-sm font-medium text-gray-700 mb-2">ชื่อประเภทบ้านเป้าหมาย <span class="text-red-500">*</span></label>
                    <input type="text" id="type_name" name="type_name" value="<?= htmlspecialchars($type['type_name']) ?>" required placeholder="เช่น บ้านค้ายาเสพติด, บ้านมั่วสุม" class="w-full border-gray-300 rounded-md shadow-sm p-2 border focus:border-blue-500 focus:ring focus:ring-blue-200">
                </div>
                ...
                <div class="flex justify-end gap-4 mt-8 pt-6 border-t border-gray-100">
                    <a href="manage_target_types.php" class="px-4 py-2 text-gray-600 hover:text-gray-900 font-medium">ยกเลิก</a>
