...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($type_name) || empty($marker_color)) {
        ...
        header("Location: target_type_form.php" . ($id > 0 ? "?id=$id" : ""));
        exit;
    }
    ...
    try {
        if ($id > 0) {
            // Update
            $stmt = $pdo->prepare("UPDATE target_types SET type_name = ?, marker_color = ? WHERE id = ?");
            $stmt->execute([$type_name, $marker_color, $id]);
            $_SESSION['success_msg'] = "อัปเดตประเภทบ้านเป้าหมายเรียบร้อยแล้ว";
        } else {
            // Insert
            $stmt = $pdo->prepare("INSERT INTO target_types (type_name, marker_color) VALUES (?, ?)");
            $stmt->execute([$type_name, $marker_color]);
            $_SESSION['success_msg'] = "เพิ่มประเภทบ้านเป้าหมายใหม่เรียบร้อยแล้ว";
        }
